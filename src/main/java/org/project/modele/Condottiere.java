    ...
    public void percevoirRessourcesSpecifiques() {
        ...
        if(super.getJoueur() != null) {
            ...
            for (int i = 0; i < super.getJoueur().getCite().size(); i++) {
                if (super.getJoueur().getCite().get(i).getType().equals("MILITAIRE")) {
                    super.getJoueur().ajouterPieces(1);
                    nbQuartiersPourBonus++;
                }
            }
            if (nbQuartiersPourBonus > 0)
                System.out.println("Vous avez gagné " + nbQuartiersPourBonus + " pièces grâce à votre personnage Condottiere car vous avez " + nbQuartiersPourBonus + " quartiers de type MILITAIRE dans votre cité.");
            else
                System.out.println("Vous n'avez pas de quartiers de type MILITAIRE dans votre cité.");
